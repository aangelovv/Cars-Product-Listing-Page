...
import CarsCard from "./VansCard";
import vanIconImage from "../../assets/van_icon.png";
import Sorter from "../Sorter/Sorter";
import Filter from "../Filter/Filter";
import Footer from "../Footer/Footer";
import vanImage from "../../assets/MB_FLEET.jpg";
...
const Vans = () => {
  ...
  const loadInitialProjects = async () => {
    try {
      const { data, count } = await vehiclesService.getAllWithCount("vans", 0);
      setCars(data);
  ...
  return (
    <>
      <article className={styles.background}>
        <img src={vanImage} alt="react-pic" className={styles.image} />
      </article>
      <div className={styles["car-collection-wrapper"]}>
        <h2 className={styles["cars-collection-heading"]}>
          Upgrade Your Game: Deliver More, Faster!
        </h2>
        <div className={styles["car-collection-img"]}>
          <img src={vanIconImage} alt="cars-collection" />
        </div>
      </div>
      <section className={styles["cars-collection"]}>
        ...
        <div className={styles["cars-counter"]}>Vans Shown: {carsCounter}</div>
      </section>
